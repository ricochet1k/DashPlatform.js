...
async function createPlatformIdentity(walletKey, coinType, identityIndex) {
    ...
    let authWallet = await DashHd.deriveIdentAuthWalletPath(walletKey, authWalletPath);
    ...
    //@ts-expect-error - monkey patch
    let regFundAddress = await DashHd.deriveIdentRegFundKeyPath(walletKey, regFundAddressPath);
    let assetAddress = await DashHd.deriveChild(regFundAddress, 0, DashHd.HARDENED);
    let topupAddressPath = `m/9'/${coinType}'/5'/2'/0`;
    //@ts-expect-error - monkey patch
    let topupAddress = await DashHd.deriveIdentTopupKeyPath(walletKey, topupAddressPath);
    console.log();
    console.log(`Identity Index: ${identityIndex}, Topup Index: 0`);
    console.log("Funding WIF", await DashHd.toWif(regFundAddress.privateKey, hdOpts));
    console.log("Asset WIF", await DashHd.toWif(assetAddress.privateKey, hdOpts), "(would be ephemeral, non-hd)");
    console.log("Auth master WIF", await DashHd.toWif(authMasterAddress.privateKey, hdOpts));
    console.log("Auth other WIF", await DashHd.toWif(authOtherAddress.privateKey, hdOpts));
    console.log("Topup WIF", await DashHd.toWif(topupAddress.privateKey, hdOpts), "(will be used for change)");
    console.log();
...
async function main() {
    ...
    if (needed > 0) {
        ...
        console.error();
        console.error(`ERROR`);
        console.error(`   not enough DASH at funding address (including instant send)`);
        console.error();
        console.error(`SOLUTION`);
    ...
    console.log(`# DEBUG funding (${fundingPkhHex}):\ntouch ./${fundingAddr}.wif`);
    ...
    console.log(`# DEBUG asset lock (${assetLockPkhHex}):\necho '${assetLockWif}' > ./${assetLockAddr}.wif`);
    ...
    let masterPrivateKeyHex = "6c554775029f960891e3edf2d36b26a30d9a4b10034bb49f3a6c4617f557f7bc";
    ...
    console.log(`# DEBUG master key (${masterPkhHex}):\necho '${masterWif}' > ./${masterAddr}.wif`);
    let otherPrivateKeyHex = "426ae4838204206cacdfc7a2e04ac6a2d9e3c2e94df935878581c552f22b0096";
    ...
    console.log(`# DEBUG other key (${otherPkhHex}):\necho '${otherWif}' > ./${otherAddr}.wif`);
     ...
     * @returns {Promise<DashBincode.AssetLockProof>}
     */
    async function getAssetLockInstantProof(magicZmqEmitter) {
        let assetLockInstantProof = {
            ...
            transaction: DashBincode.BinaryData(DashTx.utils.hexToBytes(txProofHex)),
    ...
    async function getAssetLockChainProof() {
        return DashBincode.AssetLockProof.Chain({
            // type: CHAIN_ALP,
            core_chain_locked_height: nextBlock,
            // out_point: fundingOutPointHex,
            out_point: {
                txid: DashBincode.Txid(DashTx.utils.hexToBytes(outpoint.txid)),
                vout: vout,
            },
        });
    }
    ...
    let identityKeys = await getKnownIdentityKeys({ privateKey: masterPrivateKey, publicKey: masterPublicKey }, { privateKey: otherPrivateKey, publicKey: otherPublicKey });
    ...
    console.log(`stKeys:`);
    console.log(stKeys);
    let bcAb = Bincode.encode(DashBincode.StateTransition, stateTransition, {
        signable: true,
    });
...
function signTransition(identity, assetLockProof, assetLockPrivateKey) {
    ...
    // Create ST
    const identityCreateTransition = WasmDpp.identity.createIdentityCreateTransition(identity, assetLockProof);
    // Create key proofs
    const [stMasterKey, stHighAuthKey, stCriticalAuthKey, stTransferKey] = identityCreateTransition.getPublicKeys();
    // Sign master key
    identityCreateTransition.signByPrivateKey(identityMasterPrivateKey.toBuffer(), Dpp.IdentityPublicKey.TYPES.ECDSA_SECP256K1);
    stMasterKey.setSignature(identityCreateTransition.getSignature());
    identityCreateTransition.setSignature(undefined);
    // Sign high auth key
    identityCreateTransition.signByPrivateKey(identityHighAuthPrivateKey.toBuffer(), Dpp.IdentityPublicKey.TYPES.ECDSA_SECP256K1);
    stHighAuthKey.setSignature(identityCreateTransition.getSignature());
    identityCreateTransition.setSignature(undefined);
    // Sign critical auth key
    identityCreateTransition.signByPrivateKey(identityCriticalAuthPrivateKey.toBuffer(), Dpp.IdentityPublicKey.TYPES.ECDSA_SECP256K1);
    stCriticalAuthKey.setSignature(identityCreateTransition.getSignature());
    identityCreateTransition.setSignature(undefined);
    // Sign transfer key
    identityCreateTransition.signByPrivateKey(identityTransferPrivateKey.toBuffer(), Dpp.IdentityPublicKey.TYPES.ECDSA_SECP256K1);
    ...
    identityCreateTransition.signByPrivateKey(assetLockPrivateKey, Dpp.IdentityPublicKey.TYPES.ECDSA_SECP256K1);
