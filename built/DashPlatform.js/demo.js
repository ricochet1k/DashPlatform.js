...
async function main() {
    ...
    let phrase = "wool panel expand embrace try lab rescue reason drop fog stand kangaroo";
    console.log(`Phrase:`);
    console.log(phrase);
    let salt = "";
    ...
    for (let a = 0; a <= accountIndex; a += 1) {
        ...
        for (let usage of [DashHd.RECEIVE, DashHd.CHANGE]) {
            ...
            for (let i = 0; i <= addressIndex; i += 1) {
                ...
                console.log();
                console.log(`[m/44'/${coinType}'/${a}'/${usage}/${i}] Address: ${addr}`);
    ...
    async function getAssetLockInstantProof(magicZmqEmitter) {
        let assetLockInstantProof = {
            type: INSTANT_ALP,
            instantLock: await magicZmqEmitter.once("zmqpubrawtxlocksig", 
...
async function getIdentityKeys(walletKey, idIndex) {
    ...
    let identityKey = await DashHd.deriveChild(identityEcdsaKey, idIndex, DashHd.HARDENED);
    let keyDescs = [
        {
            id: 0,
    ...
    for (let keyDesc of keyDescs) {
        let key = await DashHd.deriveChild(identityKey, keyDesc.id, DashHd.HARDENED);
