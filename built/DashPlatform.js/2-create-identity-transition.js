 ...
 */
export async function createIdentityFromAssetLock(assetKey, masterKey, otherKey, identityIdHex, txidHex, assetLockProof) {
    // const INSTANT_ALP = 0;
    // const CHAIN_ALP = 1;
    ...
    let identityKeys = await getKnownIdentityKeys({ privateKey: masterKey.privateKey, publicKey: masterKey.publicKey }, { privateKey: otherKey.privateKey, publicKey: otherKey.publicKey });
    ...
    let stateTransition = DashBincode.StateTransition.IdentityCreate(DashBincode.IdentityCreateTransition.V0(identityCreate));
    console.log(`stKeys:`);
    console.log(stKeys);
    let nullSigTransition = new Uint8Array(Bincode.encode(DashBincode.StateTransition, stateTransition, {
        signable: true,
    }));
    console.log();
    ...
    {
        let fullSigTransition = new Uint8Array(Bincode.encode(DashBincode.StateTransition, stateTransition, {
            signable: false,
        }));
        console.log();
  ...
  "stateTransition": "${grpcTransition}"
    ...
    console.log(`https://testnet.platform-explorer.com/transaction/${transitionHashHex}`);
}
;
/**
