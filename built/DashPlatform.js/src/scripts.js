...
export function makeScript(...parts) {
    let array = [];
...
export function makeP2PK(pubkey) {
    return makeScript(pubkey, OP.OP_CHECKSIG);
}
export function makeP2SH(script_hash) {
    return makeScript(OP.OP_HASH160, script_hash, OP.OP_EQUAL);
}
export function makeP2PKH(pubkey_hash) {
    return makeScript(OP.OP_DUP, OP.OP_HASH160, pubkey_hash, OP.OP_EQUALVERIFY, OP.OP_CHECKSIG);
}
export function makeOP_RETURN(data) {
    return makeScript(OP.OP_RETURN, data);
}
