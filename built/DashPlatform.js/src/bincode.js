...
export function Tuple(...inners) {
    return {
        ...
        decode(bc) {
            /** @type {*} */
            let val = new Array(inners.length);
            for (let i = 0; i < inners.length; i++) {
...
const BincodeMap = function BincodeMap(keyType, valueType) {
    return {
...
function testIt() {
    let Test = StructTuple("Foo", Uint8, String);
...
export function Struct(name, fields) {
    ...
    strct.fields = fields;
    strct.isValid = function isValid(value) {
    ...
    strct.encode = { [name + "_encode"]: function (bc, val) {
            for (const innerKey in fields) {
                fields[innerKey].encode(bc, val[innerKey]);
            }
        } }[name + "_encode"];
    strct.decode = function decode(bc) {
        /** @type {any} */
        let val = {};
        for (const innerKey in fields) {
            if (DEBUG)
                bc._debug(`  decoding ${name}.${innerKey} (${fields[innerKey].name})`);
            val[innerKey] = fields[innerKey].decode(bc);
            if (DEBUG)
                bc._debug(`  decoded ${name}.${innerKey} (${fields[innerKey].name}) to ${val[innerKey]}`);
        }
...
export function StructTuple(name, ...fields) {
    /** @type {BinCodeableStruct<T>} */
    // @ts-ignore
    const strct = {
        [name]: function (...data) {
    ...
    strct.decode = { [name + "_decode"]: function (bc) {
            /** @type {any} */
            let val = [];
            for (let innerKey = 0; innerKey < fields.length; innerKey++) {
                if (DEBUG)
...
export function match(value, fns) {
...
export function Enum(name, definitions) {
    const enumClass = { [name]: function () {
    ...
    enumClass.isValid = function isValid(value) {
    ...
    enumClass.encode = { [name + "_encode"]: function (bc, val) {
    ...
    enumClass.decode = function decode(bc) {
        const discriminant = Number(VarUint.decode(bc));
        const variantName = Object.keys(enumClass.variants)[discriminant];
        if (DEBUG)
            bc._debug(`decoded ${name} variant: ${discriminant} ${variantName}`);
        if (!variantName)
            throw new Error("Enum " + this.name + " decode failed, bad discriminant: " + discriminant);
    ...
    for (const [variantName, fields] of Object.entries(definitions)) {
        const theDiscriminant = fields[VARIANT_DISCRIMINANT] ?? prevDiscriminant;
        ...
        /** @type {*} */
        const variantClass = isTuple ? {
            [variantClassName]: function (...data) {
        ...
        variantClass.isValid = function isValid(val) {
            return variantStruct.isValid(val);
        };
        variantClass.decode = { [name + variantName + "_decode"]: function (bc) {
        ...
        variantClass.encode = { [name + variantName + "_encode"]: function (bc, val) {
...
export function VariantDiscriminant(v, discriminant) {
    v[VARIANT_DISCRIMINANT] = discriminant;
    return v;
}
export const TODO = {
    name: "TODO",
...
export function Option(inner) {
    ...
    return Object.assign({ [name]: function (data) {
            return data;
        } }[name], {
        isValid(data) {
...
export function Lazy(name, makeBincodeable) {
    /** @type {BinCodeable<T> | undefined} */
    let bincodeable = undefined;
    function unlazy() {
        if (!bincodeable)
            bincodeable = makeBincodeable();
        return bincodeable;
    }
    return {
        name,
...
Lazy.unlazy = function unlazy(val) {
    // @ts-ignore
    if ('unlazy' in val)
        return val.unlazy();
    return val;
...
export const Uint128 = {
    ...
    isValid(value) {
        if (typeof value === "bigint")
            return value >= 0n && value <= 0xffffffffffffffffffffffffffffffffn;
        return typeof value === "number" && value >= 0 && (value | 0) === value;
    },
    encode(bc, num) {
...
export const Int64 = {
    ...
    isValid(value) {
        if (typeof value === "bigint")
            return value >= -0x8fffffffffffffffn && value <= 0x8fffffffffffffffn;
        return typeof value === "number" && value >= 0 && (value | 0) === value;
    },
    encode(bc, num) {
...
export const Int128 = {
    ...
    isValid(value) {
        if (typeof value === "bigint")
            return value >= -0x8fffffffffffffffffffffffffffffffn && value <= 0x8fffffffffffffffffffffffffffffffn;
        return typeof value === "number" && value >= 0 && (value | 0) === value;
    },
    encode(bc, num) {
...
export const VarInt = {
    ...
    isValid(value) {
        if (typeof value === "bigint")
            return value >= -0xffffffffffffffffffffffffffffffffn && value <= 0xffffffffffffffffffffffffffffffffn;
...
export const Bool = {
    ...
    decode(bc) {
        ...
        if (val !== 0 && val !== 1)
            throw new Error("Bool decode error: " + val);
        return !!val;
    },
};
export const Bytes = {
