...
const E_LITTLE_INT = "JavaScript 'Number's only go up to uint53, you must use 'BigInt' (ex: `let amount = 18014398509481984n`) for larger values";
const E_TOO_BIG_INT = "JavaScript 'BigInt's are arbitrarily large, but you may only use up to UINT64 for transactions";
...
const _HEADER_ONLY_SIZE = 2 + // version
    2 + // type
    4; // locktime
export const HEADER_SIZE = 2 + // version
    2 + // type
    1 + // input count
    1 + // output count
    4; // locktime
...
export function appraise(txInfo) {
    ...
    let fees = _appraiseCounts(txInfo.inputs.length, txInfo.outputs.length, extraSize);
    return fees;
}
;
...
export function create(keyUtils) {
    ...
    if (!keyUtils.getPublicKey) {
        if (!keyUtils.toPublicKey) {
            const errGetPubKey = "you must provide 'getPublicKey()' (efficient) or 'toPublicKey()' (to convert private key)";
    ...
    txInst.hashAndSignAll = async function (txInfo, sigHashType) {
        ...
        for (let i = 0; i < sortedInputs.length; i += 1) {
            ...
            if (!isSelf) {
                console.warn(`txInfo.inputs are not ordered correctly, use txInfo.inputs.sort(Tx.sortInputs)\n(this will be an exception in the next version)`);
                break;
            }
        }
        ...
        for (let i = 0; i < sortedOutputs.length; i += 1) {
            ...
            if (!isSelf) {
                console.warn(`txInfo.outputs are not ordered correctly, use txInfo.outputs.sort(Tx.sortOutputs)\n(this will be an exception in the next version)`);
                break;
            }
        }
        /** @type {TxInfoSigned} */
        ...
        for (let i = 0; i < txInfo.inputs.length; i += 1) {
            ...
            if (privBytes) {
                txInput = await txInst.hashAndSignInput(privBytes, txInfo, i, sigHashType);
            }
     ...
     */
    txInst.hashAndSignInput = async function (privBytes, txInfo, i, sigHashType) {
        let txInput = txInfo.inputs[i];
        let _sigHashType = txInput.sigHashType ?? sigHashType ?? SIGHASH_DEFAULT;
        ...
        if ("string" !== typeof pubKeyHex) {
            ...
            console.warn(`utxo inputs should be plain JSON and use hex rather than buffers for 'publicKey'`, warn.stack);
    ...
    txInst.legacy.draftSingleOutput = function ({ utxos, inputs, output }) {
        ...
        let chanceSignaturesMatchLowestFee = Math.pow(CHANCE_INPUT_SIGS_HAVE_NO_EXTRA_BYTES, inputs.length);
        let minimumIsUnlikely = chanceSignaturesMatchLowestFee < MINIMUM_CHANCE_SIG_MATCHES_TARGET;
        ...
        let outputs = [recip];
        let change;
        let changeSats = totalAvailable + -recip.satoshis + -feeTarget + -OUTPUT_SIZE;
    ...
    txInst.legacy._signToTarget = async function (txDraft) {
        ...
        if (!hasEntropy) {
            let err = new Error("secp256k1 implementation does not use signature entropy");
    ...
    txInst.legacy._signFeeWalk = async function (txDraft) {
        ...
        for (let n = 0; true; n += 1) {
            ...
            if (!canIncreaseFee) {
                ...
                throw new Error(`no signing entropy and the fee variance is too low to cover the marginal cost of all possible signature iterations`);
            }
            let outIndex = 0;
            if (hasChange) {
            ...
            if (n >= limit) {
                throw new Error(`(near-)infinite loop: fee is ${byteFee} trying to hit target fee of ${txDraft.feeTarget}`);
            }
        }
        return txSigned;
    };
 ...
 */
export function selectSigHashInputs(txInfo, i, sigHashType = SIGHASH_DEFAULT) {
 ...
 * @param {Uint8} sigHashType
 */
export function selectSigHashOutputs(txInfo, i, sigHashType = SIGHASH_DEFAULT) {
    ...
    if (sigHashType & SIGHASH_SINGLE) {
        ...
        outputs = [output];
        throw new Error("SIGHASH_SINGLE conflicts with BIP-69, see https://github.com/dashhive/DashTx.js/issues/66");
    }
    throw new Error(`expected SIGHASH_[ALL|NONE], but got unrecognized type: ${sigHashType}`);
}
;
/**
...
export function createLegacyTx(coins, outputs, changeOutput, extraPayload) {
    ...
    if (cash < total) {
        ...
        if (cash < totalBalance) {
            throw new Error(`developer error: did not use full balance of ${totalBalance} when calculating available balance of ${cash} to pay ${total}`);
        }
        throw new Error(`balance of ${cash} cannot pay for the transaction + fees of ${total}`);
    }
    let change = 0;
...
export function createInputForSig(input, i) {
    ...
    if (!lockScript) {
        if (!input.pubKeyHash) {
            throw new Error(`signable input must have either 'pubKeyHash' or 'script'`);
        }
 ...
 * @param {Uint32?} [sigHashType]
 */
export function serialize({ version = CURRENT_VERSION, type = TYPE_VERSION, inputs, outputs, locktime = 0x0, extraPayload = "", 
/* maxFee = 10000, */
_debug = false, }, sigHashType) {
    let _sep = "";
    if (_debug) {
        _sep = "\n";
    }
...
export function serializeInput(input, i, _opts) {
    ...
    if (64 !== input.txid.length) {
        throw new Error(`expected uxto property 'txid' to be a valid 64-character (32-byte) hex string, but got '${input.txid}' (size ${input.txid.length})`);
    }
    ...
    if (isNaN(voutIndex)) {
        throw new Error(`expected utxo property 'input[${i}]outputIndex' to be an integer representing this input's previous output index`);
    }
...
function _createMemoScript(memoHex, sats, i = 0) {
    ...
    if (memoSize > 80) {
        ...
        throw new Error(`memos are limited to 80 bytes as per "Core Docs: Standard Transactions: Null Data"`);
    }
...
export function parseUnknown(txHex) {
    ...
    try {
        ...
        let msg = "parse failed, try ./DashTx.js/bin/inpsect.js <tx.hex> for detail";
        err.message += `\n${msg}`;
        throw err;
    }
    return txInfo;
...
function _parse(hex, tx = {}) {
    ...
    tx.totalSatoshis = 0;
    tx.offset = 0;
    tx.versionHex = hex.slice(tx.offset, tx.offset + 4);
    let versionHexRev = TxUtils.reverseHex(tx.versionHex);
    tx.version = parseInt(versionHexRev, 16);
    tx.offset += 4;
    tx.typeHex = hex.slice(tx.offset, tx.offset + 4);
    ...
    for (let i = 0; i < numInputs; i += 1) {
        ...
        if (0 === input.scriptSize) {
            ...
            throw new Error(`expected a "script" size of 0 (raw), 25 (hashable), or 106-109 (signed), but got '${input.scriptSize}'`);
        }
...
function assertHex(hex, propName) {
    for (let i = 0; i < hex.length; i += 1) {
        ...
        if (!isHex) {
            throw new Error(`expected each character of utxo property '${propName}' to be hex encoded but saw '${lowerChar}' in '${hex}'`);
        }
    }
}
/**
...
TxUtils._toUint32LE = function (n) {
    console.warn("warn: use public TxUtils.toUint32LE() instead of internal TxUtils._toUint32LE()");
...
TxUtils._toUint64LE = function (n) {
    console.warn("warn: use public TxUtils.toUint64LE() instead of internal TxUtils._toUint64LE()");
