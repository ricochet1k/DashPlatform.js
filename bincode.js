...
/**
 * @typedef BinCodeOptions
 * @prop {Boolean} [signable]
 */

/**
 * @template T
 * @param {BinCodeable<T>} _type
 * @param {BinCodeOptions} options
 * @param {T} value
 */
  ...
  return {
    ...
    decode(bc) {
      ...
      if (!(discriminant in definitions))
        throw new Error(
          "Enum " + name + " decode failed, bad discriminant: " + discriminant,
        );
...
export function Option(inner) {
  ...
  return Enum(name, getIndex, values);
}
...
export const IdentityPublicKeyInCreation = Enum(
...
export const AssetLockProof = Enum(
  "AssetLockProof",
  (x) => {
    return isInstantLock(x) ? 0 : 1;
  },
  {
...
/**
 * @param {any | ChainAssetLockProof | InstantAssetLockProof} x
 * @returns {x is InstantAssetLockProof}
 */
function isInstantLock(x) {
  return "instant_lock" in x;
}

export const RawAssetLockProof = Enum(
  "RawAssetLockProof",
  (x) => ("instant_lock" in x ? 0 : 1),
  {
