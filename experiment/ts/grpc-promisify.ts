...
export async function UnaryCall<M extends UnaryCall<any, any, any>>(
    client: M extends UnaryCall<infer ClientType, infer RequestType, infer ResponseType>? ClientType : never,
    method: M,
    request: M extends UnaryCall<infer ClientType, infer RequestType, infer ResponseType>? RequestType : never,
    options?: grpc.CallOptions & {signal?: AbortSignal},
): Promise<M extends UnaryCall<infer ClientType, infer RequestType, infer ResponseType>? ResponseType : never> {
...
export async function ClientStream<ClientType extends grpc.Client, RequestType, ResponseType>(
    client: ClientType,
        ...
        try {
            for await (const request of requests()) {
                await new Promise<void>((resolve, reject) => {
                    if (!stream.write(request, (error?: Error) => {
                        if (error !== undefined) {
                            reject(error);
                        }
                    })) {
